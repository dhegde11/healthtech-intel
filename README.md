...
The research prompts in [`skills/`](skills/) are plain markdown — they work with any LLM that can run web searches (Gemini, GPT-4o, Copilot, Cursor, etc.). `lookup.py` is the reference batch runner and is Anthropic-only; adapting it to another provider requires changes to the API client, tool definitions, message format, and output parsing.

### What needs to change in `lookup.py` to support another provider
...
### Sample prompt to adapt `lookup.py` with your coding assistant

Paste this into Claude, Gemini, Copilot, or Cursor to get a targeted migration plan:

```
I have a Python CLI tool called lookup.py that uses the Anthropic AsyncAnthropic SDK
...
Please read lookup.py and give me a step-by-step migration plan to replace all
